...
class AdaCNNAdaptingQLearner(object):
    ...
    def __init__(self, **params):
        ...
        self.threshold_stop_adapting = 20 # fintune should not increase for this many steps
    ...
    def output_action(self,data):
        ...
        if self.trial_phase<self.trial_phase_threshold:
            ...
            if self.trial_phase<=self.trial_phase_threshold/3.33:
                # more add actions
                # actions are indexed as [{remove actions},{add actions},{do_nothing,finetune}]
                trial_action_probs = [0.05/(1.0*self.n_conv) for _ in range(self.n_conv)] # remove
                trial_action_probs.extend([0.75 / (1.0 * self.n_conv) for _ in range(self.n_conv)]) #add
            ...
            while not found_valid_action and action_idx < self.output_size - 2:
                ...
                for li, la in enumerate(layer_actions_list):
                    ...
                    if next_filter_count <= self.min_filter_threshold or next_filter_count > self.filter_bound_vec[
                        li]:
                        self.rl_logger.info('\tAction %s is not valid li(%d), (Next Filter Count: %d). ' % (
                        ...
                        layer_actions_list = self.action_list_with_index(action_idx)
                        self.rl_logger.info('\tSelected new action: %s', layer_actions_list)
                        break
                    else:
                        found_valid_action = True
        ...
        # deterministic selection (if epsilon is not 1 or q is not empty)
            ...
            if self.use_moving_avg_q:
                q_for_actions = self.get_moving_avg_for_qs(q_for_actions)
            ...
            # We get rid of checking of Finetune is the action with Max utility because
            # now we have various parts added to reward so they stay above finetune action
            if self.trial_phase>self.trial_phase_threshold*1.5:
            ...
            if np.random.random()<0.5:
                action_idx = np.random.choice([np.argsort(q_for_actions).flatten()[-2],np.argsort(q_for_actions).flatten()[-3]])
        ...
        # random selection
            ...
            if self.global_time_stamp>self.stop_exploring_after:
                rand_indices = np.argsort(q_for_actions).flatten()[ceil(3.0*self.output_size/4.0):-1] #Only get a random index from the highest q values (except the max)
    ...
    def update_moving_avg_q(self,q_vals):
        decay = 0.5
        if len(self.moving_avg_q[0])==0:
            for q_i, q in enumerate(q_vals):
                self.moving_avg_q[q_i] = q
        else:
            for q_i, q in enumerate(q_vals):
                self.moving_avg_q[q_i] = decay*q + (1-decay)*self.moving_avg_q[q_i]


    def get_moving_avg_for_qs(self,q_queue):
        decay = 0.9
    ...
    def update_policy(self, data, add_future_reward):
        ...
        #else:
            #affected_layer_index = data['layer_index']

            #max_accuracy_push = (data['pool_accuracy'] - data['max_pool_accuracy'])/100.0
            #max_accuracy_push_new = max_accuracy_push if max_accuracy_push > 0 else 0
            #mean_accuracy =  ((data['pool_accuracy'] - data['prev_pool_accuracy']) / 100.0) + max_accuracy_push_new

        if data['prev_pool_accuracy'] == 0.0:
            mean_accuracy = 0.0
        ...
        comp_gain = self.get_complexity_penalty(data['curr_state'], data['prev_state'], self.filter_bound_vec,curr_action_string)
        reward = mean_accuracy - comp_gain

        self.rl_logger.info('Complexity penalty: %.5f',comp_gain)
        # to stop remove actions getting credits for improving accuracy
        if self.num_classes > 101 and (('add' in curr_action_string or 'remove' in curr_action_string)\
            and mean_accuracy > 0.0 and data['max_pool_accuracy']>0.0):
            mean_accuracy *= ((data['pool_accuracy']+data['prev_pool_accuracy'])/(2.0*data['max_pool_accuracy']))**2

        if self.num_classes > 101 and ('add' in curr_action_string or 'remove' in curr_action_string):
            grow_encouragement = self.get_grow_encouragement(affected_layer_index,curr_action_string,data['curr_state'],self.filter_bound_vec,5,0.5)
            reward += grow_encouragement
        ...
        if self.trial_phase>1.0:
            ...
            if 'finetune' in curr_action_string and self.same_action_count >= 1:
                self.rl_logger.info('Reward before magnification: %.5f', reward)
                # Commented to reduce the reward gotten by finetune (commented 6th sep 2017)
                #if reward > 0:
                #    reward *= min(self.same_action_count + 1,10)
                #else: # new
        ...
        if len(history_t)>=self.state_history_length:
            ...
            for invalid_a in data['invalid_actions']:
                ...
                if 'remove' in self.get_action_string(self.action_list_with_index(invalid_a)):
                    ...
                    for _ in range(3):
                        self.experience.append([history_t, self.index_from_action_list(opp_action), 1.0/(self.num_classes*2.0), history_t_plus_1,self.global_time_stamp])
                    self.reward_logger.info("%d:%d:%s:%.3f:%.3f:%.5f", self.global_time_stamp, data['batch_id'],
