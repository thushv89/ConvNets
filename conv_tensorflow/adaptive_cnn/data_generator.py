...
def get_augmented_sample_for_label(dataset_info,dataset,label,use_counter):
    ...
    if dataset_info['dataset_type'] != 'imagenet-250':
        uint8_image = dataset['dataset'][image_index,:,:,:]
    else:
        uint8_image = dataset['dataset'][image_index, :, :, :]
        uint8_image = uint8_image - np.min(uint8_image)
        uint8_image = (uint8_image*255.0/np.max(uint8_image)).astype(np.uint8)

    #print(uint8_image)
    #print(np.max(uint8_image))
    #print(np.min(uint8_image))
...
def generate_imagenet_test_data(dataset_filename,label_filename,save_directory):
    ...
    fp1 = np.memmap(filename=save_directory+os.sep+'imagenet-250-test-dataset.pkl', dtype='float32', mode='w+', shape=(test_size,resize_dim,resize_dim,num_channels))
    fp2 = np.memmap(filename=save_directory+os.sep+'imagenet-250-test-labels.pkl', dtype='int32', mode='w+', shape=(test_size,1))
...
if __name__ == '__main__':
    ...
    cdfileHandler = logging.FileHandler(persist_dir + os.sep + 'class_distribution_log' + dataset_type + distribution_type +  '.log',
                                        mode='w')
    ...
    if dataset_type == 'imagenet-250':
        ...
        if distribution_type == 'non-stationary':
            ...
            new_dataset_filename = data_save_directory + os.sep + 'imagenet-250-station-dataset.pkl'
            new_labels_filename = data_save_directory + os.sep + 'imagenet-250-station-labels.pkl'
        else:
            raise NotImplementedError
